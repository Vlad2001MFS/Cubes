...
Chunk::Chunk(hd::RenderContext &renderContext, const glm::ivec3 &pos) : mRenderContext(renderContext), mPos(pos) {
    mVertexBuffer = renderContext.createVertexBuffer(nullptr, sizeof(Vertex)*36*SIZE_X*SIZE_Y*SIZE_Z, hd::BufferUsage::Dynamic);
    mVertexCount = 0;
    mIsDirty = true;
}
    ...
    if (mIsDirty) {
        ...
        auto addFace = [&](const Vertex *verts, float x, float y, float z, float texId) {
            for (size_t i = 0; i < 6; i++) {
                auto &vertex = vertexData[mVertexCount++];
                auto &v = verts[i];
